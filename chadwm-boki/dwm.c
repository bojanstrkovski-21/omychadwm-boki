...
static int drawstatusbar(Monitor *m, int bh, char *text, int extra_offset);
...
void buttonpress(XEvent *e) {
  ...
  if (ev->window == selmon->barwin) {
    ...
    i = x = 0;
    /* left launchers (before tags) */
    for (i = 0; i < LENGTH(launchers_left); i++) {
        x += TEXTW(launchers_left[i].name);
        if (ev->x < x) {
            Arg a;
            a.v = launchers_left[i].command;
            spawn(&a);
            return;
        }
    }
    ...
    if (i < LENGTH(tags)) {
      ...
      goto execute_handler;
		...
		/* right launchers (after layout symbol) */
		for(i = 0; i < LENGTH(launchers_right); i++) {
			x += TEXTW(launchers_right[i].name);

			if (ev->x < x) {
				Arg a;
				a.v = launchers_right[i].command;
				spawn(&a);
				return;
			}
	}

  /* check launchers before systray */
  {
    unsigned int lsw_total = 0;
    unsigned int j;
    for (j = 0; j < LENGTH(launchers_systray); j++)
      lsw_total += TEXTW(launchers_systray[j].name);
    int lx = floatbar ? selmon->ww - selmon->gappov * 2 - borderpx - (int)getsystraywidth() - (int)lsw_total
                      : selmon->ww - (int)getsystraywidth() - (int)lsw_total;
    if (ev->x >= lx) {
      for (j = 0; j < LENGTH(launchers_systray); j++) {
        lx += TEXTW(launchers_systray[j].name);
        if (ev->x < lx) {
          Arg a;
          a.v = launchers_systray[j].command;
          spawn(&a);
          return;
        }
      }
...
int drawstatusbar(Monitor *m, int bh, char *stext, int extra_offset) {
  int ret, i, w, x, len;
  short isCode = 0;
  char *text;
  char *p;
  ...
  w += horizpadbar;
  if(floatbar){
    ret = x = m->ww - m->gappov * 2 - borderpx - w - extra_offset;
    x = m->ww - m->gappov * 2 - borderpx - w - getsystraywidth() - extra_offset;
  }else{
    ret = x = m->ww -  borderpx - w - extra_offset;
    x = m->ww - w - getsystraywidth() - extra_offset;
  }
...
void drawbar(Monitor *m) {
  ...
  /* calculate total width of launchers before systray */
  unsigned int lsw = 0;
  for (i = 0; i < LENGTH(launchers_systray); i++)
    lsw += TEXTW(launchers_systray[i].name);

  /* draw status first so it can be overdrawn by tags later */
  if (m == selmon) { /* status is only drawn on selected monitor */
    sw = mw - drawstatusbar(m, bh_n, stext, (int)lsw);
    /* draw launchers between status text and systray */
    int lx = floatbar ? m->ww - m->gappov * 2 - borderpx - stw - (int)lsw
                      : m->ww - stw - (int)lsw;
    for (i = 0; i < LENGTH(launchers_systray); i++) {
      w = TEXTW(launchers_systray[i].name);
      drw_setscheme(drw, scheme[SchemeLayout]);
      drw_text(drw, lx, 0, w, bh, lrpad / 2, launchers_systray[i].name, 0);
      lx += w;
    }
  }
  ...
  /* left launchers (before tags) */
  for (i = 0; i < LENGTH(launchers_left); i++) {
    w = TEXTW(launchers_left[i].name);
    drw_setscheme(drw, scheme[SchemeLayout]);
    drw_text(drw, x, 0, w, bh, lrpad / 2, launchers_left[i].name, 0);
    x += w;
  }
  for (i = 0; i < LENGTH(tags); i++) {
    w = TEXTW(tags[i]);
  ...
  x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);
  	...
  	for (i = 0; i < LENGTH(launchers_right); i++)
	{
		w = TEXTW(launchers_right[i].name);
		drw_setscheme(drw, scheme[SchemeLayout]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, launchers_right[i].name, 0);
		x += w;
	}
